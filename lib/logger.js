...
const logger = (payload = null. logLevel = 'FATAL' ) => {
    ...
    if(showMessageInDevelopment) {
        console.warn (`WARNING: Logger functon [${logLevel}] wass called while logger global error handling was not enabled (eg. logger.init() was not exeucted`)
    }
    ...
    if(window && console && document && (typeof payload === 'object' || payloadIsArray)){
        ...
        if(showMessageInDevelopment || showMessageInProduction) {
            if(console && console.hasOwnProperty($logLevel)) console[$logLevel](...loggedMessage)
            else if (console && console.log) console.log(...loggedMessage)            
        }
        ...
        try {
            ...
            request.onreadystatechange = () => {
                if(request.readyState === XMLHttpRequest.DONE && request.status !== 200) routeOnError
            }
